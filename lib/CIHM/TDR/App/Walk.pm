...
'Walks repository to find errors in structure, as well as new or missing AIPs';
...
sub run {
    ...
    if ( !$self->quiet && ( $aiplist > 0 ) ) {
        print
"There were $aiplist AIPs only found in DB.  Listing those with manifests:\n--begin--\n";
        ...
        while ( readdir $dh ) {
            ...
            if ( $depositor eq $depositordir ) {
                if ( $t_repo->aip_valid( $depositor, $identifier ) ) {
                    $self->found_aip( $poolname, $path, $depositor, $hash,
                        $identifier );
                }
                else {
                    print STDERR
                      "AIP $_ invalid at: $path/$depositor/$hash/$_\n";
                }
            }
            else {
                print STDERR
"Depositor in AIP ID didn't match depositor in path at: $path/$depositor/$hash/$_\n";
            }
        }
        closedir $dh;
    }
...
# Check if AIP already in database, but not new (possibly only set to be replicated)
            ...
            && exists $self->{aiplist}->{$aipid}->{'manifest md5'} )
        {
                ...
                $updatedoc->{'manifest md5'} )
            {
                ...
                if ( !$self->quiet ) {
                    print "MD5 mismatch $aipid: "
                      . $self->{aiplist}->{$aipid}->{'manifest md5'} . " != "
                      . $updatedoc->{'manifest md5'} . "\n";
                }
            }
    ...
    # Initialize variable -- will be noticed as mismatch, but without PERL error
                ...
                $updatedoc->{'manifest date'} )
            {
                ...
                if ( !$self->quiet ) {
                    print "Date mismatch $aipid: "
                      . $self->{aiplist}->{$aipid}->{'manifest date'} . " != "
                      . $updatedoc->{'manifest date'} . "\n";
                }
            }
    ...
    # Initialize variable -- will be noticed as mismatch, but without PERL error
            ...
            if ( $self->{aiplist}->{$aipid}->{'pool'} ne $updatedoc->{'pool'} )
            {
                if ( $self->update ) {
                    $update = 1;
                }
                if ( !$self->quiet ) {
                    print "Pool mismatch $aipid: "
                      . $self->{aiplist}->{$aipid}->{'pool'} . " != "
                      . $updatedoc->{'pool'} . "\n";
                }
            }
...
sub get_aiplist {
    ...
    $res = $t_repo->tdrepo->get(
        "/"
          . $t_repo->tdrepo->database
          . "/_design/tdr/_list/manifestinfo/tdr/repoown?reduce=false&startkey=[\"$repository\"]&endkey=[\"$repository\",{}]&include_docs=true",
        {}, {}
    );
    if ( $res->code == 200 ) {
        if ( $res->failed ) {
        ...
        print STDERR ( "get_aiplist() GET return code: "
              . $res->code
              . "\nError: "
              . $res->error
              . "\n" );
