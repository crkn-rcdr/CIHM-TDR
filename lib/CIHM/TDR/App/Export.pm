...
command_usage 'tdr export <sip|revision|metadata|dmd> [<uid> ...] [long options...]';
...
sub run {
  ...
  foreach my $param_uid (@uids) {
	  ...
	  switch ($self->cmd) {
		  ...
		  eval { $TDR->export_sip($contributor, $identifier, $target) };
		  if ($@) {
		  ...
		  if ($revision) {
		      ...
		      eval { $TDR->export_revision($contributor, $identifier, $revision, $target) };
		      if ($@) {
			  warn("Failed to export $identifier: $@\n");
		      }
		  } else {
		  ...
		  copy($metadata, $target) or die("Failed to export $metadata to $target: $!");
	      }
	      case "dmd" {
		  my $target = "$contributor.$identifier.xml";
		  eval { $TDR->export_dmd($contributor, $identifier, $target) };
		  if ($@) {
		      warn("Failed to export $identifier: $@\n");
		  }
	      }
