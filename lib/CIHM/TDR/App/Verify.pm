...
option 'limit' => (
    ...
    default => 20,
    documentation =>
      q[when selecting bags from the database set a per-request limit],
);
...
option 'workqueue' => (
    ...
    default => 2,
    documentation =>
      q[This is how you'd adjust the worker queue if you need to ],
);
...
command_short_description
  'Verifies bags from the least recently checked in the database';
...
sub run {
    ...
    my $pool =
      AnyEvent::Fork->new->require("CIHM::TDR::VerifyWorker")
        ...
        $pool->(
            ...
            sub {
                ...
                if ( $ver_res eq "ok" ) {
                    ...
                    print "verified $ver_path ($uid) in "
                      . ( $bag_end_time - $bag_start_time ) . "s\n"
                      if $verbose;
                    print "verify_time: " . $bag_stats->{verify_time} . "\n"
                      if $verbose;
                    print "size: " . $bag_stats->{size} . "\n" if $verbose;
                    $self->log->info( "verified $aippath in "
                          . ( $bag_end_time - $bag_start_time )
                          . "s checktime: "
                          . $bag_stats->{verify_time}
                          . " size: "
                          . $bag_stats->{size} );
                }
                else {
    ...
    $self->log->info( "total valid bags: "
          . $stats->{verified}->{count}
          . " invalid: "
          . $stats->{error}->{count} );
...
sub next_uid {
    ...
    if ( !exists $self->{queue} ) {
        ...
        foreach my $pool ( @{ $self->{pools} } ) {
            @{ $self->{queue}->{$pool} } =
              $self->get_pool_queue( $pool, $self->limit );
        }
...
sub get_pool_queue {
    ...
    $res = $t_repo->tdrepo->get(
        "/"
          ...
          . "/_design/tdr/_view/repopoolverified?reduce=false&startkey=[\"$repository\",\"$pool\"]&endkey=[\"$repository\",\"$pool\",{}]&limit=$limit",
        {},
    ...
    if ( $res->code == 200 ) {
        ...
        print STDERR ( "get_pool_queue($pool,$limit) GET return code: "
              . $res->code
              . "\nError: "
              . $res->error
              . "\n" );
