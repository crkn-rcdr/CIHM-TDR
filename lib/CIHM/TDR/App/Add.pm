...
option 'cosrsync' => (
  is => 'rw',
  isa => 'Bool',
  documentation => q[uid is a UID to query against a COS to then rsync into incoming and add],
);
...
parameter 'uid' => (
  ...
  isa => 'Str',
  required => 1,
  documentation => q[The uid of the AIP (In contributor.identifier form) expected in incoming],
);
...
sub run {
  ...
  if ($self->cosrsync) {
      ...
      if (!$self->replace
          && $aipinfo->{'manifest md5'} && $updatedoc->{'manifest md5'}
          && $aipinfo->{'manifest md5'} eq $updatedoc->{'manifest md5'}) {
          print STDERR "$uid with md5(". $updatedoc->{'manifest md5'} . ") already exists.\n";
          return;
      }
      $self->uid($rsyncpath);
      $self->rsync(1);
