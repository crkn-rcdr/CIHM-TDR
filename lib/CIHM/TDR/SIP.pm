...
sub validate {
    ...
    my $mets_schema;
    try {
        $mets_schema = XML::LibXML::Schema->new(location => join("/", $self->{resource}, "unpublished/xsd/mets.xsd"));
    } catch {
    ...
    my $cmets_xml = XML::LibXML->new->parse_file(join("/", $self->{resource}, "unpublished/xsl/cmets.xsl"));
    ...
    foreach my $dmdsec ($self->{metadata_xc}->findnodes("descendant::mets:dmdSec/mets:mdWrap")) {
        ...
        if ($MDTYPE eq "MARC") {
            try {
                $self->validate_node($embedded, "unpublished/xsd/MARC21slim.xsd");
            } catch {
            ...
            try {
                $self->validate_node($embedded, "unpublished/xsd/simpledc.xsd");
            } catch {
            ...
            try {
                $self->validate_node($embedded, "published/xsd/issueinfo.xsd");
            } catch {
                die "METS validation of embedded Issueinfo: $_\n";
            };
        }
        else {
            die("Found dmdSec with unsupported metadata type:  MDTYPE=\'$MDTYPE\" OTHERMDTYPE=\"$OTHERMDTYPE\"\n");
        }
    }

    # Check for the existence of the files in the METS record against the
    # files in the archive.
    foreach my $file ($self->{metadata_xc}->findnodes("/mets:mets/mets:fileSec/mets:fileGrp[\@USE='master' or \@USE='distribution']/mets:file")) {
        my $mime = $file->getAttribute('MIMETYPE');
        my $id   = $file->getAttribute('ID');
        my $href = $self->{metadata_xc}->findvalue("/mets:mets/mets:fileSec/mets:fileGrp/mets:file[\@ID='$id']/mets:FLocat[\@LOCTYPE='URN']/\@xlink:href");
        ...
        if ($filemime ne $mime) {
            die("METS record asserts that $filename should have MIME type $mime, but file seems to have $filemime\n");
        }
    }
    ...
    foreach my $file (File::Glob::bsd_glob("$self->{root}/data/files/*")) {
        ...
        die("$file contains invalid characters. Only A-Za-z0-9_.- are permitted\n") unless ($filename =~ /^[\w\.-]+$/);

        if (! $self->{metadata_xc}->findnodes("/mets:mets/mets:fileSec/mets:fileGrp/mets:file/mets:FLocat[\@xlink:href = '$filename']")) {
    ...
    foreach my $file (File::Glob::bsd_glob("$self->{root}/data/files/*")) {
        ...
        if ($suffix_type eq 'PDF') {
            my $tempdir = tempdir("CIHM::SIPXXXXXX",CLEANUP=>1, DIR => $mytemp);

            my @command=("/usr/bin/pdfseparate", # TODO: in config file?
                         $file,$tempdir."/%d.pdf");
            system(@command) == 0
            ...
            foreach my $pdffile (@pdffiles) {
                ...
                my $magic_type = qx(identify -format "%m" $tempdir/$pdffile 2> /dev/null);
                die "Distribution PDF page $pdffile type doesn't match extension\n"
            ...
            if ($xpc->exists('//txt:txtmap',$xml) || $xpc->exists('//txtmap',$xml)) {
                my $schema = XML::LibXML::Schema->new(location => "$self->{resource}/published/xsd/txtmap.xsd");
                try {
                    $schema->validate($xml);
                } catch {
                ...
                my $schema = XML::LibXML::Schema->new(location => "$self->{resource}/unpublished/xsd/alto-3-1.xsd");
                try {
                    $schema->validate($xml);
                } catch {
...
sub validate_node {
    ...
    my $schema = XML::LibXML::Schema->new(location => "$self->{resource}/$xsd");
    $schema->validate($doc);
    return 1;
}
