...
sub aip_basepath {
    ...
    if ($pool) {
        ...
        return File::Spec->rel2abs( $mypool{'aip'},
              $repo{'basepath'} . "/" . ( defined $mypool{'mountpoint'} )
            ? $mypool{'mountpoint'}
            : "" );
    }
    else {
...
sub aip_hash {
    ...
    die("Contributor code '$contributor' contains invalid characters")
      unless ( $contributor =~ /^[a-z]+$/ );
    die("Identifier '$identifier' contains invalid characters")
      unless ( $identifier =~ /^[A-Za-z0-9_]{5,64}$/ );
...
sub find_aip_pool {
    ...
    my @aip =
      ( undef, undef, $contributor, $hashcode, "$contributor.$identifier" );
    my $aippath;
    my $found;
    my $basepath;
...
sub aip_delete {
    ...
    while ( @aip = $self->find_aip_pool( $contributor, $identifier ) ) {
        ...
        my $trashpath =
          $self->trashcan_basepath( $aip[0] ) . "/$contributor.$identifier";
        return if -d $trashpath;
        rename( $aippath, $trashpath )
          or die("can't rename bag: $aippath -> $trashpath $! : $@");
...
sub aip_add {
    ...
    my $aippath =
      "$aip_basepath/$contributor/$hashcode/$contributor.$identifier";

    if ( -d $aippath ) {
        die "Can't add aip: $aippath already exists!";
    }
    make_path( dirname($aippath) );
    rename( $incomingpath, $aippath )
      or die("can't rename bag: $incomingpath -> $aippath $! : $@");
...
sub get_manifestinfo {
    ...
    open( MANIFEST, "<", $manifestpath )
      or die "Can't open manifest at $manifestpath\n";
    binmode(MANIFEST);
    $manifestdoc->{'manifest md5'} =
      Digest::MD5->new->addfile(*MANIFEST)->hexdigest;
    close(MANIFEST);
...
sub aip_add_db {
    ...
    my @aip = $self->find_aip_pool( $contributor, $identifier )
      or die "Can't find AIP $contributor.$identifier\n";

    $updatedoc =
      $self->get_manifestinfo( join( "/", @aip[ 1 .. 4 ] ), $updatedoc );
    $updatedoc->{'pool'} = $aip[0];

    if ( $self->tdrepo ) {
        $self->tdrepo->update_item_repository( "$contributor.$identifier",
            $updatedoc );
        $self->{logger}
          ->debug("updated bag in CouchDB: $contributor.$identifier");
    }
}
...
sub incoming_rsync {
    ...
    my @rsynccmd = (
        "rsync",        "-rlpt",     "--del", "--partial",
        "--timeout=10", "$source/.", "$aippath/."
    );
