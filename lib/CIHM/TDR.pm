...
sub ingest {
    ...
    die("Found existing incoming directory $aip_incoming")
      if ( -d $aip_incoming );

    if ($aip) {
        my $revision_name = strftime( "%Y%m%dT%H%M%S", gmtime(time) );
        dircopy( $aip, $aip_incoming )
          or die("Failed to copy $aip to $aip_incoming: $!");
        if ( -d "$aip_incoming/data/sip" ) {
            move( "$aip_incoming/data/sip",
                "$aip_incoming/data/revisions/$revision_name" )
              or die(
"Failed to move $aip_incoming/data/sip to $aip_incoming/data/revisions/$revision_name: $!"
              );
            dircopy( $sip_root, "$aip_incoming/data/sip" )
              or die(
                "Failed to copy content of $sip to $aip_incoming/data/sip: $!");
            $self->changelog( $aip_incoming,
            ...
            dircopy( $sip_root, "$aip_incoming/data/sip" )
              or die(
                "Failed to copy content of $sip to $aip_incoming/data/sip: $!");
            $self->changelog( $aip_incoming,
                "Created new SIP in existing AIP" );
        }
    }
    else {
        make_path("$aip_incoming/data/sip")
          or die("Failed to create $aip_incoming/data/sip: $!");
        make_path("$aip_incoming/data/revisions")
          or die("Failed to create $aip_incoming/data/revisions: $!");
        File::Copy::Recursive::rcopy_glob( "$sip_root/*",
            "$aip_incoming/data/sip" )
          or
          die("Failed to copy content of $sip to $aip_incoming/data/sip: $!");
    ...
    $t_repo->aip_delete( $contributor, $id )
      or die("Failed to move $aip to $aip_trashcan: $!");
    $t_repo->aip_add( $contributor, $id )
      or die("Failed to add $aip_incoming to repository: $!");
    return 1;
}
...
sub ingest_check {
    ...
    if ( !$aip && $update ) {
        die(    "--update requested, but AIP missing from local repository for "
              . $contributor . "."
              . $identifier
              . "\n" );
    }
            ...
            if ($aip) {
                ...
                if ($mtime) {
                    ...
                    if ( $tdrepo_mandate ne $file_mandate ) {
                        die(
"Date of $aip/manifest-md5.txt doesn't match <tdrepo> database: $file_mandate != $tdrepo_mandate"
                        );
                    }
                }
                else {
            ...
            die(
"--update requested, but AIP missing from <tdrepo> database for "
                  . $contributor . "."
                  . $identifier );
        }
    }
        ...
        while ( my $line = <$fh> ) {
            ...
            if ( substr( $line, -12 ) eq 'metadata.xml' ) {
                ...
                if ( $md5 eq $metadatamd5 ) {
                    die(
"metadata.xml from SIP matches existing AIP: $aip/$filename\n"
                    );
                }
            }
        }
...
sub update_metadata {
    ...
    my $aip_incoming =
      $t_repo->incoming_aippath( $pool, "$contributor.$identifier" );
    ...
    dircopy( $aip, $aip_incoming )
      or die("Failed to copy $aip to $aip_incoming: $!");
    mkdir("$aip_incoming/data/revisions/$revision_name")
      or die("Failed to create $aip_incoming/revisions/$revision_name: $!");
    move( "$aip_incoming/data/sip/data/metadata.xml",
        "$aip_incoming/data/revisions/$revision_name/metadata.xml" )
      or die(
"Failed tp move $aip_incoming/data/sip/data/metadata.xml to $aip_incoming/data/revisions/$revision_name/metadata.xml: $!"
      );
    copy( $metadata, "$aip_incoming/data/sip/data/metadata.xml" )
      or die(
"Failed to copy $metadata to $aip_incoming/data/revisions/$revision_name/metadata.xml: $!"
      );
    copy( "$aip_incoming/data/sip/manifest-md5.txt",
        "$aip_incoming/data/revisions/$revision_name/manifest-md5.txt" )
      or die(
"Failed to copy $aip_incoming/data/sip/manifest-md5.txt to $aip_incoming/data/revisions/$revision_name/manifest-md5.txt: $!"
      );
    ...
    # Log what we updated
    $self->changelog( $aip_incoming,
        "Updated metadata record. Reason: $reason" );

    # Generate BagIt information files for the AIP
    Archive::BagIt->make_bag($aip_incoming);

    # Move the new or updated AIP into place
    $t_repo->aip_delete( $contributor, $identifier )
      or die("Failed to move $aip to $aip_trashcan: $!");
    $t_repo->aip_add( $contributor, $identifier )
      or die("Failed to add $aip_incoming to repository: $!");
    return 1;
}
...
sub clean {
    ...
    while ( my $incomingpath =
        $t_repo->find_incoming_pool( $contributor, $identifier ) )
    {
        print "Removing: $incomingpath\n";
        remove_tree($incomingpath) or die("Failed to remove $incomingpath: $!");
    }
    while ( my $trashpath =
        $t_repo->find_trash_pool( $contributor, $identifier ) )
    {
...
sub delete {
    ...
    dircopy( $aip, $aip_incoming )
      or die("Failed to copy $aip to $aip_incoming: $!");
    foreach my $file ( bsd_glob("$aip_incoming/data/revisions/*"),
        "$aip_incoming/data/sip", "$aip_incoming/data/cmr.xml" )
    {
        if ( -d $file ) {
    ...
    $self->changelog( $aip_incoming,
        "Deleted SIP and all revisions from archive. Reason: $reason" );
...
sub changelog {
    ...
    open( CHANGELOG, ">>$aip/data/changelog.txt" )
      or die("Failed to open $aip/data/changelog.txt for writing: $!");
